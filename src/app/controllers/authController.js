  ...
  try {
    if (await User.findOne({ email }))
      return res.status(400).send({
        error:
          "Atenção: Este e-mail já está cadastrado, para obter acesso ao sistema, clique em recuperar senha e siga os procedimentos enviados para sua caixa de entrada. Verifique a caixa de SPAM."
      });
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user.tipo == "empresa") {
      empresa = await Empresa.findOne({ usuario: user._id }).populate(
        "usuario"
      );
      if (!empresa)
      ...
      cliente = await Cliente.findOne({ usuario: user._id }).populate(
        "usuario"
      );
